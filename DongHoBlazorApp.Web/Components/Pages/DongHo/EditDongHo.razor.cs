...
{
    public partial class EditDongHo
    {
        [Inject]
        ...
        protected override async Task OnInitializedAsync()
        {
            try
            {
                if (Id > 0)
                {
                    ...
                    var taskDongHo = ApiClient.GetFromJsonAsync<BaseReponseModel>($"/api/DongHo/dongHoId?dongHoId={Id}");
                    ...
                    await Task.WhenAll(taskDongHo, taskPhanLoai, taskThuongHieu, taskNhaCungCap);

                    var dongHo = await taskDongHo;
                    var phanLoai = await taskPhanLoai;
                    var thuongHieu = await taskThuongHieu;
                    var nhaCungCap = await taskNhaCungCap;

                    if (dongHo != null && dongHo.Success)
                    {
                        DongHoModel = JsonConvert.DeserializeObject<DongHoModel>(dongHo.Data.ToString());
                    }
        ...
        public async Task ConfirmEditDongHo()
        {
            IsSuccess = false;
            IsError = false;
            ...
            try
            {
                ...
                DongHoModel.MaDongHo = Id; // Ensure ID is set correctly
                var res = await ApiClient.PutAsJsonAsync<BaseReponseModel, DongHoModel>("/api/DongHo", DongHoModel);
