...
{
    public partial class IndexNhaCungCap
    {
        [Inject]
        ...
        protected override async Task OnInitializedAsync()
        {
            try
            {
                var res = await ApiClient.GetFromJsonAsync<BaseReponseModel>("/api/NhaCungCap");
                if (res != null && res.Success)
                {
                    NhaCungCapModels = JsonConvert.DeserializeObject<List<NhaCungCapModel>>(res.Data.ToString());
                }
        ...
        public async Task ConfirmDelete()
        {
            try
            {
                var res = await ApiClient.DeleteAsync<BaseReponseModel>($"/api/NhaCungCap?DeleteId={DeleteId}");
                if (res != null && res.Success)
                {
